...
public class HelperBeanGenerator extends AbstractGenerator implements Generator<GeneratorConfig,Descriptor>{
    ...
    protected boolean generateInternal(GeneratorConfig generatorConfig, Descriptor descriptor) {
        ...
        beanClass.setClassName(ResourceWrapper.JavaUtil.getJavaClassName(platformName) + "Helper");
        ...
        for (Rule rule : ruleList) {
            ...
            if(text != null && text.trim().startsWith("//TODO")) {
                ...
                for (String string : strings) {
                    String paramName = ResourceWrapper.JavaUtil.getJavaVarName(string.substring(2, string.length() - 1));
                    if(string.startsWith("#")) {
                        text = text.replace(string, ResourceWrapper.JavaUtil.getJavaClassName(platformName) + "Config.getInstance().get"
