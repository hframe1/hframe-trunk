...
public class ClientBeanGenerator extends AbstractGenerator implements Generator<GeneratorConfig,Descriptor>{
    ...
    protected boolean generateInternal(GeneratorConfig generatorConfig, Descriptor descriptor) {
        ...
        beanClass.setClassName(ResourceWrapper.JavaUtil.getJavaClassName(platformName) + "Client");
        ...
        for (Interface anInterface : interface1List) {
            ...
            method.addCodeLn("if(\"true\".equals(" + ResourceWrapper.JavaUtil.getJavaClassName(platformName) + "Config.getInstance().getTestModel())) {");
    ...
    private String getFormatUrl(Method method, Interface anInterface, List<Parameter> parentParameterList) {
        ...
        String url = "String url = UrlHelper.getUrlPath(" +
                ResourceWrapper.JavaUtil.getJavaClassName(platformName) + "Config.getInstance().get"
...
//        url += ", parameterMap";
            ...
            if(StringUtils.isNotBlank(value)) {
                ...
                if(strings != null && strings.length > 0) {
                    ...
                    if(strings[0].startsWith("#")) {
                        return ResourceWrapper.JavaUtil.getJavaClassName(platformName) + "Config.getInstance().get"
    ...
    private String getMethodInvokeCode(Method method1) {
        ...
        String code = ResourceWrapper.JavaUtil.getJavaClassName(platformName) + "Helper."
                + method1.getName() + "(" ;
            ...
            if(ruleNodeList != null && ruleNodeList.size() > 0) {
                ...
                if(Arrays.binarySearch(new String[]{"String","int","long",""},type) < 0) {
                    if("Map".equals(type)) {
                        ...
                        field.setName(ResourceWrapper.JavaUtil.getJavaVarName(subNode == null ? "String" : subNode.getType()));
                    }
                }
            }
        }
                            ...
                            if (childXmlNode.isSingleton()) {
                                if(StringUtils.isNotBlank(value)) {
                                    ...
                                    if(strings != null && strings.length > 0) {
                                        String paramName = strings[0].substring(2, strings[0].length() - 1);
                                        if(strings[0].startsWith("#")) {
                                            method.addCodeLn("   " + ResourceWrapper.JavaUtil.getJavaVarName(childXmlNode.getNodeName()) +
                                                    "=" + ResourceWrapper.JavaUtil.getJavaClassName(platformName) + "Config.getInstance().get"
                                                    + ResourceWrapper.JavaUtil.getJavaClassName(paramName) + "();");
                                        }
                                    }
                                ...
                                if(StringUtils.isNotBlank(ruleId)) {
                                    method.addCodeLn("   " + ResourceWrapper.JavaUtil.getJavaVarName(childXmlNode.getNodeName()) +
                                            "=" + ResourceWrapper.JavaUtil.getJavaClassName(platformName) + "Helper."
                                            + ruleId + "(" + whetherContainThis(ruleId) + ");");
                                }
                                modifyMethod(beanClass, ResourceWrapper.JavaUtil.getJavaVarName(childXmlNode.getNodeName()));
                            }
                        }
                    }
                }
