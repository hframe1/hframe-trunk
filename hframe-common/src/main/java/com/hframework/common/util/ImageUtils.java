...
public class ImageUtils {
     ...
     */
    public final static byte[] scale(String srcImageFile, String result,
        ...
        try {
            ...
            g.dispose();
            bytes= ImageUtils.imageByArray(tag);
    ...
    /**
     * 压缩图片并保存
     * @param multipartFile 所要压缩的图片
     * @param path          压缩图片后的路径
     * @param memberId       账户
     * @return   操作成功/失败
     * @throws IOException
     */
     public final static Map<String,Object> saveImage(MultipartFile multipartFile,String path,String memberId) throws IOException {
         FileOutputStream outStream=null;
         Map<String,Object> map= new HashMap<String, Object>();
         try{
             InputStream is=multipartFile.getInputStream();
             byte[] data =StringUtils.readInputStream(is);
             File imageFile=new File(path);  //图片压缩后保存的路径
             if (!imageFile.exists()){
                 imageFile.mkdirs();
             }
             //String test = multipartFile.getOriginalFilename();
             String suffix=multipartFile.getOriginalFilename().substring(multipartFile.getOriginalFilename().lastIndexOf("."));//获取原文件后缀
             //String filename = memberId + new Date().getTime();
             File file = new File(path,memberId+"_big"+suffix); //原图片保存路径
             FileUtils.createFile(file.getAbsolutePath()); //没有路径则创建
             outStream = new FileOutputStream(file);
             outStream.write(data);
             byte[] bytes= ImageUtils.scale(file.getAbsolutePath(), imageFile.getAbsolutePath() + File.separator +memberId+"_small"+suffix, 5, false);//压缩图片
             map.put("result","y");
             map.put("bytes",bytes);
             return map;
         }catch (Exception e){
