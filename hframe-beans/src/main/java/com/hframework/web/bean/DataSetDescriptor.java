            ...
            for (DataSetHelper dataSetHelper : dataSetHelpers) {
                ...
                if(StringUtils.isNotBlank(helpDatascore)) {
                    ...
                    for (String propertyName : condition.keySet()) {
                        ...
                        String referDataFiledPropertyName = ResourceWrapper.JavaUtil.getJavaVarName(referDataFiledCode);
                        ...
                        for (Mapping mapping : mappingList) {
                            if("true".equals(mapping.getIsCompareKey())) {
                                object.put("compareKey", ResourceWrapper.JavaUtil.getJavaVarName(mapping.getEffectDatasetField()));
                            }
                            if("true".equals(mapping.getIsCompareName())) {
                                object.put("compareName", ResourceWrapper.JavaUtil.getJavaVarName(mapping.getEffectDatasetField()));
                            }
                        }
        ...
        for (DataSetRuler dataSetRuler : dataSetRulers) {
            ...
            if(ruleList != null) {
                ...
                for (Rule rule : ruleList) {
                    String sourceCode = ResourceWrapper.JavaUtil.getJavaVarName(rule.getSourceCode());
                    String sourceValue = rule.getSourceValue();
                    String targetCode = ResourceWrapper.JavaUtil.getJavaVarName(rule.getTargetCode());
