...
public class ControllerHelper {
    ...
    public static <T> T setRelationFieldValue(T object) throws Exception {
        ...
        for (String fieldCode : relFieldKeyMap.keySet()) {
            String propertyName = ResourceWrapper.JavaUtil.getJavaVarName(fieldCode);
            ...
            boolean fillResult = WebContext.fillProperty(
                    Class.forName(relPoClass.getClassPath()).getName(), object, propertyName, ResourceWrapper.JavaUtil.getJavaVarName(relFieldCode));
            if(!fillResult) {
                //从页面流上线文中获取数据
                WebContext.fillProperty(Map.class.getName(), object, propertyName, ResourceWrapper.JavaUtil.getJavaVarName(relFieldCode));
            }
        }
        //对页面及页面流中对象的对象进行赋值
        for (String fieldCode : relFieldKeyMap.keySet()) {
...
//                String fieldCode = ResourceWrapper.JavaUtil.getJavaVarName(field.getCode());
...
//                //从页面上下文中获取数据
//                boolean fillResult = WebContext.fillProperty(
//                        Class.forName(relPoClass.getClassPath()).getName(), object, fieldCode, ResourceWrapper.JavaUtil.getJavaVarName(relFieldCode));
//                if(!fillResult) {
//                    //从页面流上线文中获取数据
//                    WebContext.fillProperty(Map.class.getName(), object, fieldCode, ResourceWrapper.JavaUtil.getJavaVarName(relFieldCode));
//                }
//
//                if(!fillResult) {
//
...
////                Object relPo = WebContext.get(Class.forName(relPoClass.getClassPath()).getName());
////                if(relPo != null) {
////                    String relFieldName = ResourceWrapper.JavaUtil.getJavaVarName(relFieldCode);
