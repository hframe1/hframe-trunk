...
public class HfClassContainerUtil {
	...
	public static HfClassContainer getClassInfoContainer(HfModelContainer container) {
		...
		Collection<HfmdEntity> hfmdEntitys = container.getEntityMap().values();
		for (HfmdEntity hfmdEntity : hfmdEntitys) {
			String hfmdEntityCode = hfmdEntity.getHfmdEntityCode();
			String className = ResourceWrapper.JavaUtil.getJavaClassName(hfmdEntityCode);
		...
		for (String key : entityAttrMap.keySet()) {
			...
			String className = ResourceWrapper.JavaUtil.getJavaClassName(entityCode);
			HfClassContainer.HfClassDescriptor classInfo = hfClassContainer.getClassInfo(className);
			HfmdEntityAttr entityAttr = entityAttrMap.get(key);
			classInfo.addField(entityAttr.getHfmdEntityAttrCode(),
					ResourceWrapper.JavaUtil.getJavaVarName(entityAttr.getHfmdEntityAttrCode()),
