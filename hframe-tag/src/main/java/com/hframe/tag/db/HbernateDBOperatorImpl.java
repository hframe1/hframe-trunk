 ...
 */
@Deprecated
public class HbernateDBOperatorImpl implements IDBOperator{
...
//        AbstractDAOSupport AbstractDAOSupport= (AbstractDAOSupport) webappcontext.getBean("abstractDAOSupport");
//        return AbstractDAOSupport.find(" from "+ResourceWrapper.JavaUtil.getJavaClassName(dbObjectName)+" where  "+ResourceWrapper.JavaUtil.getJavaVarName(primaryKey)+" = '"+objectId+"'");
    ...
    public List getObjectsByCondition(String condition, String dbObjectName, int pageIndex, int pageSize) {
        ...
        //Object service=webappcontext.getBean(TagUtils.getJavaVarName(dbObjectName)+"Serv");
//		Class c=Class.forName(className);
//
//        AbstractDAOSupport AbstractDAOSupport= (AbstractDAOSupport) webappcontext.getBean("abstractDAOSupport");
//        return AbstractDAOSupport.find(" from " + ResourceWrapper.JavaUtil.getJavaClassName(dbObjectName) + " where  " + condition, (pageIndex - 1) * pageSize, pageSize);
//        //return AbstractDAOSupport.find(" from "+TagUtils.getJavaClassName(dbObjectName)+" where  "+condition+"  LIMIT "+pageIndex+", "+pageSize);
        ...
        try {
            ...
            if(pageIndex > 0 && pageSize > 0) {
                ReflectUtils.invokeMethod(
                        poExampleObj, "setLimitStart", new Class[]{Integer.class}, new Object[]{(pageIndex - 1) * pageSize+1});
                ReflectUtils.invokeMethod(
...
//        AbstractDAOSupport AbstractDAOSupport= (AbstractDAOSupport) webappcontext.getBean("abstractDAOSupport");
//
//        return AbstractDAOSupport.find(" from "+ResourceWrapper.JavaUtil.getJavaClassName(dbObjectName)+" where  "+condition ).size();
...
//        List lastList=AbstractDAOSupport.find(" from "+ResourceWrapper.JavaUtil.getJavaClassName(dbObjectName)+" where  "+condition );
//        result.addAll(lastList);
//
//        if(result!=null&&result.size()==1){
//            findTreeObjects(result,lastList,dbObjectName,AbstractDAOSupport,id,pid,popath);
//        }
    ...
    private void findTreeObjects(List result, List lastList, String dbObjectName,String id, String pid,
        ...
        for (Object object : lastList) {
            Object parValue= ReflectUtil.invokeGetMethod(id, object, Class.forName(poPath));
...
//    private void findTreeObjects(List result, List lastList, String dbObjectName,AbstractDAOSupport AbstractDAOSupport, String id, String pid,  String popath) throws SecurityException, IllegalArgumentException, NoSuchMethodException, IllegalAccessException, InvocationTargetException, ClassNotFoundException {
//        for (Object object : lastList) {
//            Object parValue=ReflectUtil.invokeGetMethod(id, object, Class.forName(popath));
//            String parName=ResourceWrapper.JavaUtil.getJavaVarName(pid);
//            List tempResult=AbstractDAOSupport.find(" from "+ResourceWrapper.JavaUtil.getJavaClassName(dbObjectName)+" where  "+parName+ " = "+ parValue);
//            result.addAll(tempResult);
//            findTreeObjects(result,tempResult,dbObjectName,AbstractDAOSupport,id,pid,popath);
//        }
//    }
...
//        AbstractDAOSupport abstractDAOSupport= (AbstractDAOSupport) webappcontext.getBean("abstractDAOSupport");
//        if(param==null){
//            List list=abstractDAOSupport.getJdbcTemplate().queryForList(sql);
//            return list;
//        }else{
//
//            List list=abstractDAOSupport.getJdbcTemplate().queryForList(sql,param);
//            return list;
//        }
//        return null;
    }
...
//
//        String[] sqlParts=new String[4];
//        List sqlList=abstractDAOSupport.getJdbcTemplate().queryForList("select * from core_element where core_element_id='"+sqlId+"'");
